...
class TMgramNet(nn.Module):
    def __init__(self, num_layer=3, mel_bins=128, win_len=1024, hop_len=512):
        super(TMgramNet, self).__init__()
        # if "center=True" of stft, padding = win_len / 2
        self.conv_extrctor = nn.Conv1d(1, win_len//2+1, win_len, hop_len, win_len // 2, bias=False)
        self.mel_transform = torchaudio.transforms.MelScale(n_mels=mel_bins,
                                                            sample_rate=16000,
                                                            n_stft=win_len//2+1)
    def forward(self, x):
        out = self.conv_extrctor(x)
        out = self.mel_transform(out)
        return out


class SgramNet(nn.Module):
    def __init__(self, num_layer=3, mel_bins=128, win_len=1024, hop_len=512):
        super(SgramNet, self).__init__()
        # if "center=True" of stft, padding = win_len / 2
        self.spec_transform = torchaudio.transforms.Spectrogram(n_fft=win_len,
                                                                win_length=win_len,
                                                                hop_length=hop_len,
                                                                power=2.)
        self.conv_encoder = nn.Sequential(
            *[nn.Sequential(
                nn.LayerNorm(313),
                nn.LeakyReLU(0.2, inplace=True),
                nn.Conv1d(win_len//2+1 if idx==0 else mel_bins, mel_bins, 3, 1, 1, bias=False)
            ) for idx in range(num_layer)])
    def forward(self, x):
        out = self.spec_transform(x).squeeze(1)
        out = self.conv_encoder(out)
        return out
...
class STgramMFN(nn.Module):
    def __init__(self, num_classes, num_attributes,
        ...
        # self.tmgramnet = TMgramNet(mel_bins=c_dim, win_len=win_len, hop_len=hop_len)
        # self.sgramnet = SgramNet(mel_bins=c_dim, win_len=win_len, hop_len=hop_len)
        self.mobilefacenet = MobileFaceNet(num_class=num_classes,
                                           bottleneck_setting=bottleneck_setting)
        self.attribute_head = nn.Linear(128, num_attributes)
    ...
    def forward(self, x_wav, x_mel, label):
        ...
        # x_tm = self.tmgramnet(x_wav).unsqueeze(1)
        x = torch.cat((x_mel, x_t), dim=1)
        # x = torch.cat((x_mel, x_tm, x_t), dim=1)
        out, feature = self.mobilefacenet(x, label)
        out_atts = self.attribute_head(feature)
        if self.arcface:
            out = self.arcface(feature, label)
        return out, out_atts, feature
...
if __name__ == '__main__':
    ...
    import torch
    net = STgramMFN(num_classes=270, num_attributes=171)
    input1 = torch.randn(1, 160000)
    input2 = torch.randn(1, 128, 313)
    input3 = None
