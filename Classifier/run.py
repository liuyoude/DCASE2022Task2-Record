...
def main(args):
    ...
    trainer.valid(valid_dirs=args.valid_dirs, save=args.save, decision=args.decision)
    trainer.eval(train_dirs=args.train_dirs, test_dirs=args.test_dirs)
...
def run():
    ...
    # args.version = f'STgram_MFN_ASLloss(alpha=norm(w),pos=2,neg=2,eps=0.1)_addSecAtt'
    args.version = f'STgram_MFN_Focalloss'
    args.description = 'focal loss(alpha=norm(w))'
    ...
    if args.test:
        args.version = f'2022-05-01-16-STgram_MFN_FocalLoss(alpha=norm(w))-ft-target'
        log_dir = f'runs/{args.version}'
    ...
    print(args.version, args.test, args.device_ids)
    main(args)
