...
def get_label(filename, att2idx, file_att_2_idx):
    atts = filename2attributes(filename)
    # file_att = '_'.join(atts)
    file_att = filename2section(filename)
    file_label = file_att_2_idx[file_att]
    one_hot = torch.zeros((len(att2idx.keys())))
    for att in atts: one_hot[att2idx[att]] = 1
    return file_label, one_hot
...
def filename2attributes(filename):
    ...
    name_list = os.path.splitext(name)[0].split('_')[6:]
...
def filename2section(filename):
    name_list = []
    machine, name = filename.split('/')[0], filename.split('/')[2]
    name_list.append(machine)
    name_list.extend(os.path.splitext(name)[0].split('_')[:2])
...
def get_file_sections(dir_path, machine=None, section=None, domain=None):
    file_sections = set()
    state = {}
    sep = system_sep
    filename_list = get_filename_list(dir_path, ext='wav')
    for filename in filename_list:
        ...
        filename = filename.split(sep)[-3:]
        filename = '/'.join(filename)
        file_section = filename2section(filename)
        file_sections.add(file_section)
        if file_section not in state.keys():
            state[file_section] = 1
        else:
            state[file_section] += 1
    return sorted(file_sections), state


def map_attribuate(dir_path):
    attributes, _ = get_attributes(dir_path)
    idx2att, att2idx = {}, {}
...
def map_file_attribute(dir_path):
    file_attribute_2_idx = {}
    idx_2_file_attribuate = {}
    file_attributes, _ = get_file_attributes(dir_path)
...
def map_file_section(dir_path):
    file_section_2_idx = {}
    idx_2_file_section = {}
    file_sections, _ = get_file_sections(dir_path)
    for idx, file_section in enumerate(list(file_sections)):
        file_section_2_idx[file_section] = idx
        idx_2_file_section[idx] = file_section
    return file_section_2_idx, idx_2_file_section
...
def cal_anomaly_score(probs, machine_section_file_atts, file_att_2_idx):
    ...
    anomaly_score = - np.log10(np.sum(releated_probs) + eps)
...
def cal_anomaly_score_att(probs, att_probs, machine_section_file_atts, file_att_2_idx, att2idx):
    ...
    anomaly_score = np.mean(- np.log10(releated_att_probs + eps))
    return anomaly_score
...
def data_statistics():
    dir_path = './dcase2022dataset/dev_data'
    ...
    section_list = ['section_00', 'section_01', 'section_02']
...
if __name__ == '__main__':
    ...
    # data_statistics()
    filename = 'bearing/test/section_00_source_test_anomaly_0000_vel_6.wav'
    print(filename2section(filename))
