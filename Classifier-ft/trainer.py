...
class FTTester:
    def __init__(self, *args, **kwargs):
        self.args = kwargs['args']
        self.s_net = kwargs['s_net']
        self.t_net = kwargs['t_net']
        self.optimizer = kwargs['optimizer']
        self.scheduler = kwargs['scheduler']
        self.writer = self.args.writer
        self.logger = self.args.logger
    ...
    def valid(self, valid_dirs, save=True, result_dir=None):
        s_net = self.s_net.module if self.args.dp else self.s_net
        t_net = self.t_net.module if self.args.dp else self.t_net
        s_net.eval()
        t_net.eval()
        ...
        for index, target_dir in enumerate(sorted(valid_dirs)):
            ...
            for section_str in machine_section_list:
                csv_path = os.path.join(result_dir, f'anomaly_score_{machine_type}_{section_str}.csv')
                machine_section_file_atts, _ = utils.get_file_attributes(target_dir, machine=machine_type,
                                                                           section=section_str)
                test_files, y_true, domain_list = utils.get_valid_file_list(target_dir, section_str)
                y_pred = [0. for _ in test_files]
                anomaly_score_list = []
                for file_idx, file_path in enumerate(test_files):
                    inputs, label, one_hot = self.transform(file_path)
                    with torch.no_grad():
                        s_out_classes, out_atts, _ = s_net(*inputs, label)
                        t_out_classes, out_atts, _ = t_net(*inputs, label)
                    s_probs = torch.softmax(s_out_classes, dim=1).mean(dim=0).squeeze().cpu().numpy()
                    t_probs = torch.softmax(t_out_classes, dim=1).mean(dim=0).squeeze().cpu().numpy()
                    s_score = utils.cal_anomaly_score(s_probs, machine_section_file_atts,
                                                               self.args.file_att_2_idx)
                    t_score = utils.cal_anomaly_score(t_probs, machine_section_file_atts,
                                                      self.args.file_att_2_idx)
                    y_pred[file_idx] = max(s_score, t_score)
                    # att_probs = torch.sigmoid(out_atts).mean(dim=0).squeeze().cpu().numpy()
                    # y_pred[file_idx] = utils.cal_anomaly_score_att(probs, att_probs, machine_section_file_atts,
                    #                                                self.args.file_att_2_idx, self.args.att2idx)
                    anomaly_score_list.append([os.path.basename(file_path), y_pred[file_idx]])
